  ...
  try {
    ...
    if (existingLocation.rows.length === 0) {
      const addLocationQuery =
        'INSERT INTO locations (location) VALUES ($1) RETURNING _id';
    ...
    const updateUserQuery =
      'UPDATE users SET trips = array_append(trips, $1) WHERE username = $2';
...
tripController.deleteTrip = async (req, res, next) => {
  ...
  try {
    ...
    const updateUserTripsQuery =
      'UPDATE users SET trips = $1 WHERE username = $2';
